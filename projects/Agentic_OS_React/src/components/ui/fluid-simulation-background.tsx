...
// ─── High-Fidelity Fluid Simulation Background ───
// Based on Navier-Stokes equations for realistic smoke/fluid effects.
// Adapted from the user-provided code for seamless integration into Agentic OS.

export const FluidSimulationBackground = () => {
  ...
  const config = {
    ...
    PAUSED: false,
    BACK_COLOR: { r: 0.01, g: 0.01, b: 0.02 }, // Dark background
    TRANSPARENT: true
  };
  ...
  if (error) return null; // Silent fail if WebGL not supported

  return (
    <div className="fixed inset-0 z-[6] pointer-events-none mix-blend-screen opacity-80">
      <canvas
        ref={canvasRef}
