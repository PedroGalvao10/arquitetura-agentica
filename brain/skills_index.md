### CATEGORIA: CORE (Processos & Pesquisa)
- investigação_factual: Verificação de leis, dados e cruzamento de informações reais.
- pesquisa_profunda: Metodologia estruturada para exploração exaustiva de temas complexos.
- planejamento_reverso: Quebra de objetivos finais em tarefas atômicas e sequenciais.
- solução_de_problemas: Protocolo de diagnóstico e correção de bugs (Self-Annealing).
- prompt_engineering_ai: Guia mestre consolidado de engenharia de prompt agnóstico.
- skill-creator: Framework para criar novas habilidades e diretivas estruturadas e escaláveis.
- brainstorming: Processo de ideação e design antes da implementação técnica.
- find-skills: Descoberta de novas habilidades e ferramentas na comunidade.

### CATEGORIA: UI & DESIGN (Experiência Premium)
- antigravity-design-expert: Interfaces fluidas, 3D CSS, GSAP, Glassmorphism e profundidade.
- ui-ux-pro-max: Guia completo de design, acessibilidade, paletas e sistemas visuais.
- site-architecture: Planejamento de hierarquia, navegação, URLs e linking interno.
- canvas-design: Estratégias para criar artefatos visuais e interativos de alta fidelidade com IA.
- theme-factory: Geração de sistemas de design, tokens e paletas de cores harmônicas.
- algorithmic-art: Criação de arte generativa e componentes visuais complexos via código.
- brand-guidelines: Manutenção de identidade visual, tom de voz e consistência de marca.

### CATEGORIA: CODING (Desenvolvimento & Performance)
- react: Melhores práticas de React, hooks, performance e arquitetura de componentes.
- react-best-practices: Otimização de performance e carregamento em frameworks modernos.
- frontend-design: Princípios de design de interface com foco em estética premium e UX fluida.
- mcp-builder: Guia e protocolos para construção de Model Context Protocol (MCP) servers.
- ai-api: Integração técnica, otimização de chamadas e roteamento inteligente de APIs de IA.
- supabase: Melhores práticas de performance e design de banco de dados Postgres via Supabase.
- web-artifacts-builder: Metodologia para construção de componentes web prontos para produção.
- webapp-testing: Estratégias rigorosas de testes de interface e validação de jornada do usuário.
- api-retry: Decoradores de retry com exponential backoff para resiliência de chamadas LLM e APIs.

### CATEGORIA: MARKETING & VISIBILIDADE
- seo: Auditoria técnica, on-page, schema markup e prontidão para IA (GEO).
- copywriting: Escrita persuasiva e conversão para páginas de vendas e landing pages.
- content-strategy: Planejamento estratégico de conteúdo buscável e compartilhável.
- ai-product-photography: Geração de fotografia de produto profissional via IA.
- ai-marketing-videos: Criação de vídeos promocionais e anúncios de marketing via IA.

### CATEGORIA: ESPECIALIZADA
- nutrition-analyzer: Análise técnica de dados nutricionais e saúde personalizada.
- docx: Automação, processamento e geração de documentos Word estruturados.
- pdf: Extração precisa e formatação de dados complexos de/para arquivos PDF.
- xlsx: Manipulação avançada de planilhas Excel e análise de dados estruturados.
- pptx: Geração automática de apresentações e slides profissionais de alto impacto.
- internal-comms: Protocolos de redação para comunicações internas e documentação técnica.
- slack-gif-creator: Criação rápida de ativos visuais e animações para comunicação ágil.
- doc-coauthoring: Estratégias de fluxos de trabalho colaborativos para escrita e revisão dinâmica.

### CATEGORIA: BLUEPRINTS (Códigos Reutilizáveis)
- video_scroll_control: Controle de vídeo por scroll via Canvas API.
- backend_auth_sqlite: Servidor Express com SQLite e autenticação JWT.
- leads_automation: Automação de captura de leads para Google Sheets.



### CATEGORIA: UI/UX & DESIGN CRAFT
- interface-design-core: Fundamentos e princípios de design engineering e refinamento visual.
- interface-design-audit: Auditoria técnica de padrões visuais e consistência de componentes.
- interface-design-critique: Revisão estética de alto nível para elevar o 'craft' de interfaces.
- interface-design-extract: Extração de tokens e regras de design system a partir de código existente.
- frontend-design-creative: Diretiva para criação de interfaces premium com impacto visual e animações.
- site-clone-reference: Recompilação de sites a partir de URLs, extraindo estrutura, estilos e ativos.

### CATEGORIA: IMPORTADAS DOS DOWNLOADS
...
- aws-serverless-eda: Aws Serverless Eda
...
### CATEGORIA: IMPORTADAS DOS DOWNLOADS (673 items)
...
- debug-buttercup: Debugs the Buttercup CRS (Cyber Reasoning System) running on Kubernetes. Use when diagnosing pod crashes, restart loops, Redis failures, resource pressure, disk saturation, DinD issues, or any service misbehavior in the crs namespace. Covers triage, log analysis,...
...
- differential-review: Performs security-focused differential review of code changes (PRs, commits, diffs). Adapts analysis depth to codebase size, uses git history for context, calculates blast radius, checks test coverage, and generates comprehensive markdown reports. Automatically...
...
- fixing-metadata: Audit and fix HTML metadata including page titles, meta descriptions, canonical URLs, Open Graph tags, Twitter cards, favicons, JSON-LD structured data, and robots directives. Use when adding SEO metadata, fixing social share previews, reviewing Open Graph tags,...
...
- frontend-slides: Create stunning, animation-rich HTML presentations from scratch or by converting PowerPoint files. Use when the user wants to build a presentation, convert a PPT/PPTX to web, or create slides for a talk/pitch. Helps non-designers discover their aesthetic through visual...
...
- geoffrey-hinton: Agente que simula Geoffrey Hinton — Godfather of Deep Learning, Prêmio Turing 2018, criador do backpropagation e das Deep Belief Networks. Use quando quiser: perspectivas históricas sobre deep...
...
- golang-security-auditor: Golang Security Auditor
...
- jupyter-workflow: Jupyter Workflow
...
- llm-ops: LLM Operations -- RAG, embeddings, vector databases, fine-tuning, prompt engineering avancado, custos de LLM, evals de qualidade e arquiteturas de IA para producao. Ativar para: implementar RAG,...
...
- monte-carlo-treasury: Monte Carlo Treasury
- monte-carlo-vulnerability-detection: Monte Carlo Vulnerability Detection
...
- observe-whatsapp: Observe Whatsapp
...
- proof-of-vulnerability: Proof Of Vulnerability
...
- publish-and-summary: Publish And Summary
...
- python-security-auditor: Python Security Auditor
...
- analise-instagram: Skill para analise instagram
- avoid-ai-writing: Skill para avoid ai writing
- carrossel-instagram: Skill para carrossel instagram
- cortes-de-video: Skill para cortes de video
- dashboard-metricas: Skill para dashboard metricas
- hooks-magneticos: Skill para hooks magneticos
- link-in-bio: Skill para link in bio
- roteiro-pauta-quente: Skill para roteiro pauta quente
